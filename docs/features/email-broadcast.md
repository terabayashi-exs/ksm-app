...
| `scheduleAnnouncement` | 大会日程・組合せ決定通知 | 大会日程・試合組合せ決定時の通知 |
...
- **プレースホルダー**:
  - `[ここに記載]` - 編集箇所を明示
  - `[URLをここに記載]` - **自動的に大会詳細URLに置換される**（例: `https://rakusyo-go.com/tournaments/123`）
...
### Phase 1（現在） - 完了
- ✅ 基本的なメール送信機能
- ✅ 8種類のプリセット（参加確定、辞退承認、大会日程・組合せ決定通知含む）
- ✅ 5チーム選択制限
- ✅ BCC方式
- ✅ メールアドレス重複除去
- ✅ URL自動置換（`[URLをここに記載]`）
...
### 2026年1月9日
- 大会日程・組合せ決定通知テンプレート追加（8種類目）
- テンプレート表示名の日本語化対応
- スパムフィルタ回避対応（「【重要】」「重要なお知らせ」を削除）
- メールテンプレートの組織名表示統一

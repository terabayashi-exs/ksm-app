...
### **📊 システム構成（2026年1月時点）**
- **総テーブル数**: 24テーブル（マスター8 + トランザクション15 + その他1）
- **新規追加テーブル**: 12テーブル（初期設計後に追加）
  - `m_sport_types`: スポーツ種別対応
  - `m_subscription_plans`: サブスクリプションプラン管理
  - `t_announcements`: お知らせ管理（NEW: 2026-01-08）
  ...
  - `t_administrator_subscriptions`: 管理者サブスクリプション情報
  - `t_subscription_usage`: サブスクリプション使用状況
  - `t_payment_history`: 支払い履歴
  - `sample_data`: テストデータ用

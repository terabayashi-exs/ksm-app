...
CIが通らないPRはマージできません（ブランチ保護ルール設定済み）。
CIが通り、レビューが完了したらdevにマージします。

> **補足: PRとdevの関係**
> PRを作成した時点ではdevには影響しません。PRは「マージしてください」という提案であり、マージを実行するまでdevのコードは変わりません。
> fixブランチ上で `npm run dev` を実行すれば、devとほぼ同じ環境で動作確認できます。

### レビュー担当者の確認手順

他のメンバーが出したPRを、ローカル環境で動作確認してからマージする手順です。
...
# PRのブランチをチェックアウト
git checkout fix/qr-validity-display
```
...
この時点ではdevには影響していないため、安心してテストできます。

#### 3. 結果に応じた対応

**問題なし → GitHub上でPRをマージ**

GitHub上のPRページで「Merge pull request」をクリックします。

**問題あり → PRにコメントで修正依頼**

PRのコメント欄に具体的な修正内容を記載します。開発者が修正してpushすると、同じPRに反映されます。

...
async function main() {
  ...
  const tournaments = await db.execute(
    "SELECT tournament_id, tournament_name, format_id, preliminary_format_type, final_format_type, phases FROM t_tournaments",
  );
  let tUpdated = 0;
  for (const t of tournaments.rows) {
    if (t.preliminary_format_type && t.final_format_type) continue;
    const format = await db.execute({
      sql: "SELECT preliminary_format_type, final_format_type, phases FROM m_tournament_formats WHERE format_id = ?",
      args: [t.format_id],
    });
    ...
    if (f.phases === null || f.phases === undefined || phasesValue === "null") {
      const phases = [];
      if (f.preliminary_format_type)
        phases.push({
          id: "preliminary",
          order: 1,
          name: "予選",
          format_type: f.preliminary_format_type,
        });
      if (f.final_format_type)
        phases.push({
          id: "final",
          order: 2,
          name: "決勝トーナメント",
          format_type: f.final_format_type,
        });
  ...
  try {
    await db.execute({
      ...
      args: [20, "0016_template_independence", 1740672000000],
    });

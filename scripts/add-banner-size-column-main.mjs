// t_sponsor_bannersテーブルにbanner_sizeカラムを追加 (本番環境用)
import { createClient } from '@libsql/client';
import * as dotenv from 'dotenv';

dotenv.config({ path: '.env.local' });

// 本番環境の接続情報（コメントアウトされている行から取得）
const MAIN_DB_URL = 'libsql://ksm-main-asditd.aws-ap-northeast-1.turso.io';
const MAIN_DB_TOKEN = 'eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJhIjoicnciLCJpYXQiOjE3NTY0NjIwODcsImlkIjoiOTYxYzExMWMtZWRkZS00NGFhLTljZmYtMWE3OTBkNDZjMDQzIiwicmlkIjoiZjBlZGMzZDgtYzBhYS00ZjBjLThiMWYtNDdiN2JmMTQ4Y2JiIn0.rWO9p3UvjUKogEB7Dd3YQKiIBkHzgdL8xaXISyZkw9nPfOlQ_mlNIAkMK6YP3DIRNPQhvDmReLNqpC4-ayJ1Bg';

const client = createClient({
  url: MAIN_DB_URL,
  authToken: MAIN_DB_TOKEN
});

async function addBannerSizeColumn() {
  console.log('🔧 [PRODUCTION] t_sponsor_bannersテーブルにbanner_sizeカラムを追加中...\n');
  console.log('⚠️  本番環境への変更を実行します...\n');

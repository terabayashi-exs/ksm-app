...
async function copyMainToDev() {
  try {
    ...
    for (const tr of allTournaments.rows) {
      ...
      console.log(`  - tournament_id: ${(tr as any).tournament_id}: ${rules.rows.length}件のルール`);
    }
    ...
    if (g.admin_login_id) {
      ...
      if (adminExists.rows.length === 0) {
        console.log(`⚠️  管理者 ${g.admin_login_id} がdev環境に存在しません。admin_login_idをNULLにして作成します。`);
    ...
    if (g.login_user_id) {
      ...
      if (userExists.rows.length === 0) {
        console.log(`⚠️  ログインユーザー ${g.login_user_id} がdev環境に存在しません。login_user_idをNULLにして作成します。`);
    ...
    if (g.venue_id) {
      ...
      if (venueExists.rows.length === 0) {
        console.log(`⚠️  会場 venue_id:${g.venue_id} がdev環境に存在しません。mainからコピーします...`);
        ...
        if (venue.rows.length > 0) {
          ...
          await devDb.execute({
            ...
            args: [
              v.venue_id, v.venue_name, v.address, v.prefecture_id,
              v.available_courts, v.google_maps_url, v.latitude, v.longitude,
            ],
          });
    ...
    if (existingGroup.rows.length === 0) {
      ...
      await devDb.execute({
        ...
        args: [
          g.group_id, g.group_name, g.organizer, g.venue_id,
          g.event_start_date, g.event_end_date,
          g.recruitment_start_date, g.recruitment_end_date,
          g.visibility, g.event_description, g.admin_login_id, g.login_user_id,
        ],
      });
    ...
    for (const formatId of formatIds) {
      ...
      if (formatExists.rows.length === 0) {
        console.log(`⚠️  フォーマット format_id:${formatId} がdev環境に存在しません。mainからコピーします...`);
        ...
        if (format.rows.length > 0) {
          ...
          await devDb.execute({
            ...
            args: [
              f.format_id, f.format_name, f.target_team_count,
              f.format_description, f.phases, f.visibility,
            ],
          });
    ...
    for (const tr of allTournaments.rows) {
      ...
      await devDb.execute({
        ...
        args: [
          td.tournament_id, td.tournament_name, td.format_id, td.format_name || null,
          td.venue_id, td.team_count, td.court_count,
          td.tournament_dates, td.match_duration_minutes, td.break_duration_minutes,
          td.status, td.visibility, td.public_start_date,
          td.recruitment_start_date, td.recruitment_end_date,
          td.sport_type_id, td.created_by, td.archive_ui_version,
          td.is_archived || 0, td.files_count || 0,
          td.group_order, td.category_name, td.group_id,
          td.show_players_public || 0, td.phases ? JSON.stringify(td.phases) : null,
        ],
      });
    ...
    for (const ruleSet of allRules) {
      for (const rule of ruleSet.rules) {
        ...
        if (existingRule.rows.length > 0) {
          console.log(`  ⚠️  tournament_id:${ruleSet.tournamentId}, phase:${r.phase} のルールは既に存在。スキップ。`);
          continue;
        }
        ...
        await devDb.execute({
          ...
          args: [
            ruleSet.tournamentId, r.phase, r.use_extra_time, r.use_penalty,
            r.active_periods, r.notes, r.point_system, r.walkover_settings,
            r.tie_breaking_rules, r.tie_breaking_enabled,
          ],
        });
      }
      if (ruleSet.rules.length > 0) {
        console.log(`✅ ルールを作成: tournament_id=${ruleSet.tournamentId} (${ruleSet.rules.length}件)`);
      }
    }
    ...
    for (const tr of allTournaments.rows) {
      ...
      if (displaySettings.rows.length > 0) {
        console.log(`✅ 表示設定をコピー: tournament_id=${td.tournament_id} (${displaySettings.rows.length}件)`);
      }
    }

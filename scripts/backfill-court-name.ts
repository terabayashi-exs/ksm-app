...
async function run() {
  ...
  if (courtMappings.rows.length > 0) {
    ...
    for (const row of courtMappings.rows) {
      const result = await db.execute({
        ...
        args: [row.court_name as string, row.court_number as number, row.tournament_id as number, row.court_number as number],
      });
      if (result.rowsAffected > 0) {
        console.log(`  ✓ tournament_id=${row.tournament_id}, court_number=${row.court_number} → "${row.court_name}": ${result.rowsAffected}件更新`);
      }
    }
  } else {
  ...
  for (const row of verify.rows) {
    console.log(`  court_number=${row.court_number}, court_name=${JSON.stringify(row.court_name)}, count=${row.cnt}`);
  }

...
async function migratePrefectures(env: string) {
  ...
  try {
    ...
    // 現在のレコード数を確認
    const countResult = await client.execute("SELECT COUNT(*) as count FROM m_prefectures");
    ...
    // 登録後の確認
    const finalCountResult = await client.execute("SELECT COUNT(*) as count FROM m_prefectures");

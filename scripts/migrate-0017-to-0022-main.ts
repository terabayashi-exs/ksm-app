...
async function recordMigration(id: number, tag: string, when: number) {
  try {
    await db.execute({ sql: 'INSERT INTO __drizzle_migrations (id, hash, created_at) VALUES (?, ?, ?)', args: [id, tag, when] });
...
async function main() {
  ...
  const r0017 = await db.execute('UPDATE t_tournaments SET format_name = (SELECT format_name FROM m_tournament_formats WHERE m_tournament_formats.format_id = t_tournaments.format_id) WHERE format_name IS NULL');
  ...
  const tournaments = await db.execute('SELECT DISTINCT format_id, match_duration_minutes, break_duration_minutes FROM t_tournaments WHERE format_id IS NOT NULL');
  ...
  console.log('\n=== 0020: remove phase check constraint ===');
  const createSql = await db.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name='t_tournament_rules'");
  ...
  try {
    ...
    await db.execute("UPDATE t_tournaments SET venue_id = '[' || venue_id_legacy || ']' WHERE venue_id_legacy IS NOT NULL AND (venue_id IS NULL OR venue_id = '')");

...
const db = createClient({
  url: process.env.TURSO_DATABASE_URL || "libsql://ksm-prod-asditd.aws-ap-northeast-1.turso.io",
  authToken: process.env.TURSO_AUTH_TOKEN || "eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJhIjoicnciLCJpYXQiOjE3MzQyMzc1MjMsImlkIjoiZGM3NGZiYzEtOWUxOC00MGM2LWIzMWMtMWRmZjU1NDg1NjJlIiwicmlkIjoiYWRmMWM2NDYtYWJhZS00OTJkLWI5N2UtMTM1MjgzOGE2N2Y1In0.dJdXTSlpHPMPSSnW7xFcR5QYXI-cTW8uLHEYUyW3y5FqcZLLEu2hfqTFLc8yw0l8Wz97aKGUZvniFN9ILX7zAA"
});

async function migrate() {
  console.log('🚀 マイグレーション開始: win_condition カラム削除 (本番環境)\n');
  console.log('⚠️  本番環境のデータベースに対して実行します');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
  ...
  try {
    ...
    const totalCount = await db.execute(`SELECT COUNT(*) as count FROM t_tournament_rules`);
    console.log(`   合計: ${totalCount.rows[0].count} レコード\n`);
    ...
    console.error('⚠️  必要に応じてバックアップから復元してください。');
    process.exit(1);
  }
}

// 実行前の確認
console.log('\n⚠️  本番環境のデータベースに対してマイグレーションを実行します');
console.log('⚠️  実行を続けるには Ctrl+C で中断するか、そのまま待機してください...\n');

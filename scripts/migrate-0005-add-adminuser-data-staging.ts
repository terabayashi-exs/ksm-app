/**
 * マイグレーション 0005: 管理者ユーザーデータ移行スクリプト
 *
 * dev環境のlogin_user_id:1のデータをstaging環境に複製します
 * - m_login_users
 * - m_login_user_roles
 ...
 *
 * 使用方法:
 *   npx tsx scripts/migrate-0005-add-adminuser-data.ts
 *
 * 注意:
 *   - dev環境からstaging環境へのコピーのみ対応
...
async function main() {
  ...
  try {
    ...
    // ステップ2: staging環境に既存データがあるかチェック
    console.log("📋 [STEP 2] staging環境の既存データをチェック中...\n");

    const existingUserResult = await stagDb.execute({
    ...
    console.log("✓ staging環境に既存データはありません\n");
    ...
    if (sourceLogoUrl) {
      console.log("📤 [STEP 3] Blobのロゴを複製中...\n");
      ...
      try {
        ...
        // staging環境のBlobにアップロード
        console.log("staging環境のBlobにアップロード中...");
        const filename =
          user.logo_filename || sourceLogoUrl.split("/").pop() || `admin-logo-${Date.now()}.png`;

        const uploadResult = await put(filename, buffer, {
          access: "public",
          token: process.env.BLOB_READ_WRITE_TOKEN, // staging環境のトークン
        });
    ...
    // ステップ4: staging環境にユーザーデータを登録
    console.log("💾 [STEP 4] staging環境にデータを登録中...\n");

    // m_login_users
    console.log("m_login_users に登録中...");
    await stagDb.execute({
      sql: `
    ...
    if (roles.length > 0) {
      ...
      for (const role of roles) {
        await stagDb.execute({
          ...
          `,
          args: [role.login_user_id, role.role, role.created_at],
        });
    ...
    if (authorities.length > 0) {
      ...
      for (const auth of authorities) {
        await stagDb.execute({
          ...
          `,
          args: [auth.login_user_id, auth.tournament_id, auth.permissions, auth.created_at],
        });
        console.log(
          `  ✓ 権限登録: tournament_id=${auth.tournament_id}, permissions=${auth.permissions}`,
        );
      }
    ...
    const verifyResult = await stagDb.execute({
      sql: `SELECT * FROM m_login_users WHERE login_user_id = ?`,
      args: [1],
    });

    if (verifyResult.rows.length === 0) {
      console.error("❌ エラー: staging環境にデータが登録されませんでした");
      process.exit(1);
    }

    const verifiedUser = verifyResult.rows[0] as unknown as LoginUser;
    console.log("✓ staging環境に正常に登録されました:");
    ...
    console.log(`Blobロゴ: ${newLogoBlobUrl ? "複製済み" : "なし"}\n`);
  } catch (error) {

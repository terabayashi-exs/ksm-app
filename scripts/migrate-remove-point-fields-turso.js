...
async function createBackup() {
  ...
  try {
    ...
    const backupData = {
      ...
      description: 'Migration backup before removing point fields (Turso version)',
      table: 't_tournaments',
      data: tournaments.rows
    };
    
    const backupFileName = `backup-tournaments-turso-${new Date().toISOString().slice(0, 19).replace(/[:.]/g, '-')}.json`;
  ...
  try {
    ...
    for (const row of currentData.rows) {
      ...
      `, [
        ...
        row.walkover_winner_goals, row.walkover_loser_goals,
 ...
 */
async function main() {
  console.log('🎯 勝点フィールド削除マイグレーション開始（Turso版）');
  console.log('===============================================');

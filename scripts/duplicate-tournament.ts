...
async function duplicateTournament() {
  try {
    ...
    for (const [oldTournamentId, newTournamentId] of tournamentIdMap.entries()) {
      ...
      console.log(
        `トーナメントルールを複製: tournament_id ${newTournamentId} (${rules.rows.length}件)`,
      );
    }
    ...
    for (const [oldTournamentId, newTournamentId] of tournamentIdMap.entries()) {
      ...
      for (const tt of tournamentTeams.rows) {
        ...
        if (teamIdMap.has(teamData.team_id)) {
          ...
          console.log(
            `チームマスターを複製: ${teamData.team_name} (${teamData.team_id} -> ${newTeamId})`,
          );
        }
    ...
    for (const [oldTeamId, newTeamId] of teamIdMap.entries()) {
      ...
      for (const player of players.rows) {
        ...
        await db.execute({
          ...
          args: [p.player_name, p.jersey_number, newTeamId, p.is_active],
        });
        totalPlayers++;
      }
    ...
    for (const [oldTournamentId, newTournamentId] of tournamentIdMap.entries()) {
      ...
      if (matchesLive.rows.length > 0) {
        console.log(
          `試合データ(t_matches_live)を複製: tournament_id ${newTournamentId} (${matchesLive.rows.length}件)`,
        );
      }
    }

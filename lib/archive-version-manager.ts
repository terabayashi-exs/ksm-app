...
export class ArchiveVersionManager {
  ...
  static async getArchiveUIVersion(tournamentId: number): Promise<string> {
    ...
    return (
      (version as string) || this.inferVersionFromDate(result.rows[0].archived_at as string | null)
    );
  }
  ...
  private static inferVersionFromDate(archivedAt: string | null): string {
    ...
    const sortedVersions = [...ARCHIVE_VERSIONS.VERSION_HISTORY].sort(
  ...
  static async getArchiveInfo(tournamentId: number): Promise<{
    ...
    const versionInfo = row.archive_ui_version
      ? getVersionInfo(row.archive_ui_version as string)
      : undefined;

...
export async function calculateTournamentStatus(
  ...
  if (tournament.status === "completed" && tournamentId && !allMatchesCompleted) {
    console.log(
      `🔄 Tournament ${tournamentId}: Status overridden from 'completed' to 'ongoing' (${allMatchesCompleted ? "all matches confirmed" : "matches pending"})`,
    );
    return "ongoing";
  }
  ...
  if (tournamentId) {
    try {
      ...
      if (matchBasedOngoing) {
        console.log(
          `🏁 Tournament ${tournamentId}: Status set to 'ongoing' because matches have started`,
        );
        return "ongoing";
      }
    } catch (error) {
  ...
  if (recruitmentStart && recruitmentEnd && today >= recruitmentStart && today <= recruitmentEnd) {
    return "recruiting";
  }
 ...
 */
export function calculateTournamentStatusSync(tournament: {
  status: string;
  tournament_dates: string;
  recruitment_start_date: string | null;
  recruitment_end_date: string | null;
...
async function checkTournamentHasOngoingMatches(tournamentId: number): Promise<boolean> {
  try {
    ...
    console.log(
      `Tournament ${tournamentId}: ${startedCount} matches have started (not scheduled, excluding BYE matches)`,
    );
...
async function checkAllMatchesCompleted(tournamentId: number): Promise<boolean> {
  try {
    ...
    console.log(
      `Tournament ${tournamentId}: ${completedMatches}/${totalMatches} matches completed (confirmed or cancelled, excluding BYE matches)`,
    );

 ...
 */
async function fetchAllMatchTimes(
...
async function fetchAllMatchDates(
  tournamentIds: number[],
): Promise<Map<number, { startDate: string; endDate: string }>> {
...
function groupTournaments(tournaments: Tournament[]): GroupedTournamentData {
  ...
  Object.values(grouped).forEach((group) => {
    group.tournaments.sort((a, b) =>
      (a.tournament_name || "").localeCompare(b.tournament_name || "", "ja"),
    );
  });
...
export async function fetchOperatorDashboardData(
  ...
  // 動的ステータスを並列計算
  const tournamentsWithTimes: Tournament[] = await Promise.all(
  ...
  // グループステータスを決定
  const groupStatuses: Record<
    number,
    "planning" | "recruiting" | "before_event" | "ongoing" | "completed"
  > = {};
  tournamentsWithTimes.forEach((t) => {
    if (t.group_id && !groupStatuses[t.group_id]) {
      ...
      else if (group.some((x) => x.status === "before_event"))
        groupStatuses[t.group_id] = "before_event";
      else if (group.some((x) => x.status === "recruiting"))
        groupStatuses[t.group_id] = "recruiting";
      else if (group.some((x) => x.status === "planning")) groupStatuses[t.group_id] = "planning";
      else if (group.every((x) => x.status === "completed"))
        groupStatuses[t.group_id] = "completed";
      else groupStatuses[t.group_id] = "planning";
    }
  });

  const filterByStatus = (status: string) =>
    tournamentsWithTimes.filter((t) =>
      t.group_id ? groupStatuses[t.group_id] === status : t.status === status,
    );

...
async function processMatchAdjustments(withdrawalInfo: WithdrawalInfo): Promise<void> {
  ...
  for (const match of relatedMatches.rows) {
    ...
    const opponentTournamentTeamId = isTeam1
    ...
    if (match.match_status === "scheduled") {
      ...
      if (opponentTournamentTeamId) {
        await processWalkoverMatch(
          matchId,
          Number(opponentTournamentTeamId),
          withdrawalInfo,
          String(match.match_code),
        );
      } else {
...
async function processWalkoverMatch(
  ...
  `,
    [
      withdrawalInfo.tournament_team_id,
      withdrawalInfo.tournament_team_id,
 ...
 * エラーログの記録
 */
async function logWithdrawalError(tournamentTeamId: number, error: Error | unknown): Promise<void> {

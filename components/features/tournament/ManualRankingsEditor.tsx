...
import { AlertTriangle, MessageSquare, RotateCcw, Save, Trophy } from "lucide-react";
...
export default function ManualRankingsEditor({
  tournamentId,
  blocks,
  phases,
  finalMatches = [],
  ...
  const getPhaseFormatType = useCallback(
  ...
  const calculateFinalRankings = (): FinalRanking[] => {
    ...
    const quarterFinalMatches = finalMatches.filter((m) =>
      ["T1", "T2", "T3", "T4"].includes(m.match_code),
    );
    ...
    if (finalMatch?.is_confirmed && finalMatch.winner_tournament_team_id) {
      ...
      const team1IdStr = finalMatch.team1_tournament_team_id
        ? String(finalMatch.team1_tournament_team_id)
        : null;
      const team2IdStr = finalMatch.team2_tournament_team_id
        ? String(finalMatch.team2_tournament_team_id)
        : null;
      ...
      if (winnerId) {
        rankings.push({
          team_id: winnerId,
          team_name:
            team1IdStr === winnerId ? finalMatch.team1_display_name : finalMatch.team2_display_name,
          position: 1,
          is_confirmed: true,
        });
      }
      ...
      if (loserId) {
        rankings.push({
          team_id: loserId,
          team_name:
            team1IdStr === loserId ? finalMatch.team1_display_name : finalMatch.team2_display_name,
          position: 2,
          is_confirmed: true,
        });
      }
    ...
    if (thirdPlaceMatch?.is_confirmed && thirdPlaceMatch.winner_tournament_team_id) {
      ...
      const team1IdStr = thirdPlaceMatch.team1_tournament_team_id
        ? String(thirdPlaceMatch.team1_tournament_team_id)
        : null;
      const team2IdStr = thirdPlaceMatch.team2_tournament_team_id
        ? String(thirdPlaceMatch.team2_tournament_team_id)
        : null;
      const loserId = team1IdStr === winnerId ? team2IdStr : team1IdStr;
      ...
      if (winnerId) {
        rankings.push({
          ...
          team_name:
            team1IdStr === winnerId
              ? thirdPlaceMatch.team1_display_name
              : thirdPlaceMatch.team2_display_name,
          position: 3,
          is_confirmed: true,
        });
      }
    ...
    semiFinalMatches.forEach((match) => {
      if (match.is_confirmed && match.winner_tournament_team_id) {
        ...
        const team1IdStr = match.team1_tournament_team_id
          ? String(match.team1_tournament_team_id)
          : null;
        const team2IdStr = match.team2_tournament_team_id
          ? String(match.team2_tournament_team_id)
          : null;
    ...
    teamSet.forEach((teamId) => {
      if (!rankedTeamIds.has(teamId)) {
        ...
        const team1IdStr = teamMatch?.team1_tournament_team_id
          ? String(teamMatch.team1_tournament_team_id)
          : null;
        ...
        if (finalMatch) {
          const finalTeam1IdStr = finalMatch.team1_tournament_team_id
            ? String(finalMatch.team1_tournament_team_id)
            : null;
          const finalTeam2IdStr = finalMatch.team2_tournament_team_id
            ? String(finalMatch.team2_tournament_team_id)
            : null;
        ...
        if (defaultPosition === 5 && thirdPlaceMatch) {
          const thirdTeam1IdStr = thirdPlaceMatch.team1_tournament_team_id
            ? String(thirdPlaceMatch.team1_tournament_team_id)
            : null;
          const thirdTeam2IdStr = thirdPlaceMatch.team2_tournament_team_id
            ? String(thirdPlaceMatch.team2_tournament_team_id)
            : null;
        ...
        if (
          defaultPosition === 5 &&
          semiFinalMatches.some((m) => {
            const team1IdStr = m.team1_tournament_team_id
              ? String(m.team1_tournament_team_id)
              : null;
            const team2IdStr = m.team2_tournament_team_id
              ? String(m.team2_tournament_team_id)
              : null;
            return team1IdStr === teamId || team2IdStr === teamId;
          })
        ) {
  ...
  const hasManualAdjustmentNeeded = (teamRankings: TeamRanking[], block: Block): boolean => {
    ...
    // 決勝進出に必要な順位のみチェック
    const teamsInRequiredPositions = teamRankings.filter((team) =>
      requiredPositions.has(team.position),
    );
  ...
  const resetBlock = (blockIndex: number) => {
    ...
    setMessage({
      type: "success",
      text: `${blocks[blockIndex].block_name}ブロックをリセットしました`,
    });
  };
  ...
  const saveChanges = async () => {
    ...
    const requestData = {
      ...
      finalTournament:
        getPhaseFormatType(activeTab) === "tournament" && finalMatches.length > 0
          ? {
    ...
    console.log(
      `[MANUAL_RANKINGS_FRONTEND] ${phaseNameMap.get(activeTab) || activeTab}タブの送信データ:`,
    ...
    blocksToSave.forEach((block) => {
      ...
      block.team_rankings.forEach((editedTeam) => {
        const originalTeam = originalBlock?.team_rankings.find(
          (t) => t.team_id === editedTeam.team_id,
        );
        if (originalTeam && editedTeam.position !== originalTeam.position) {
          console.log(
            `  順位変更: ${editedTeam.team_name} ${originalTeam.position}位 → ${editedTeam.position}位`,
          );
        }
      });
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList
          ...
          style={{ gridTemplateColumns: `repeat(${phaseList.length}, minmax(0, 1fr))` }}
        >
          {phaseList.map((phase) => (
        ...
        {phaseList.map((phase) => (
          <TabsContent key={phase.id} value={phase.id} className="space-y-6 mt-6">
            {/* リーグ形式のブロック一覧 */}
            {editedBlocks.filter(
              (block) => block.phase === phase.id && !block.block_name.endsWith("_unified"),
            ).length > 0 && (
              <div className="grid gap-6 lg:grid-cols-2">
                {editedBlocks
                  ...
                  .map((block) => {
                    const blockIndex = editedBlocks.findIndex(
                      (b) => b.match_block_id === block.match_block_id,
                    );
                    const needsAdjustment = hasManualAdjustmentNeeded(block.team_rankings, block);
                    return (
                      <Card
                        key={block.match_block_id}
                        className={`h-fit ${needsAdjustment ? "border-2 border-yellow-400 shadow-lg" : ""}`}
                      >
                        <CardHeader className={needsAdjustment ? "bg-yellow-50" : ""}>
                          <CardTitle className="flex items-center justify-between">
                            <div className="flex items-center">
                              <div
                                className={`px-3 py-1 rounded-full text-sm font-medium mr-3 ${getBlockColor(block.block_name)}`}
                              >
                        ...
                        <CardContent>
                                ...
                                .map((team, teamIndex) => (
                                  ...
                                  >
                                    <div className="flex items-center gap-2">
                                      <Label
                                        htmlFor={`position-${block.match_block_id}-${team.team_id}`}
                                    ...
                                    <div className="flex-1">
                                      ...
                                      {getPhaseFormatType(block.phase) === "league" && (
                                        <div className="text-xs text-gray-600">
                                          {team.points}pt ({team.wins}W {team.draws}D {team.losses}
                                          L) 得失点差: {team.goal_difference > 0 ? "+" : ""}
                          ...
                          <div className="mt-4 pt-4 border-t border-gray-200">
                            <Label
                              htmlFor={`remarks-${block.match_block_id}`}
              ...
              <div className="space-y-4">
                <Card className="h-fit">
                  ...
                  <CardContent>
                    {finalTournamentBlock.team_rankings.length === 0 ? (
                      ...
                      <div className="space-y-3">
                        {finalTournamentBlock.team_rankings
                          ...
                          .map((team, teamIndex) => (
                            <div
                              key={`tournament-${team.team_id}-${teamIndex}`}
                            ...
                            >
                              <div className="flex items-center gap-2">
                                <Label
                                  htmlFor={`tournament-position-${team.team_id}`}
                                  className="text-sm font-medium"
                                >
                                  順位:
                                </Label>
                                <Input
                                  ...
                                  value={team.position}
                                  onChange={(e) =>
                                    updateFinalPosition(teamIndex, parseInt(e.target.value) || 1)
                                  }
                                  className="w-16 h-8 text-center"
                                  disabled={saving}
                                />
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-200">
                      <Label
                        htmlFor={`tournament-remarks-${phase.id}`}
                        className="flex items-center text-sm font-medium text-gray-700 mb-2"
                      >
                        <MessageSquare className="w-4 h-4 mr-1" />
                        トーナメント順位設定の備考
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur border-t z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex gap-3">
          ...
          <Button onClick={saveChanges} disabled={saving || !hasChanges} className="flex-1">

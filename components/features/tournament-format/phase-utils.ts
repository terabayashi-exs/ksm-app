...
import type { TournamentPhase, TournamentPhases } from "@/lib/types/tournament-phases";

/**
 * フェーズ設定の拡張バリデーション（UI専用）
 */
export function validatePhaseConfiguration(phases: TournamentPhases): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  if (invalidIds.length > 0) {
    errors.push(
      `不正なフェーズID: ${invalidIds.map((p) => p.id).join(", ")} (英小文字、数字、アンダースコアのみ使用可能)`,
    );
  }
...
export const PHASE_PRESETS: Record<string, { name: string; phases: TournamentPhases }> = {
  ...
  single_phase_final: {
    name: "単一フェーズ（決勝のみ）",
    phases: {
      phases: [{ id: "final", order: 1, name: "決勝トーナメント", format_type: "tournament" }],
    },
  },
  single_phase_preliminary: {
    name: "単一フェーズ（予選のみ）",
    phases: {
      phases: [{ id: "preliminary", order: 1, name: "予選", format_type: "league" }],
    },
  },
};

    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 p-4 space-y-4">
          <h3 className="text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">基本設定</h3>

          <div className="space-y-3">
            <div>
              <label className="text-xs text-gray-600 dark:text-gray-400 mb-1 block">チーム数</label>
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  className="w-16 h-8 bg-white dark:bg-slate-800 rounded-lg text-center text-sm font-bold border border-gray-300 dark:border-slate-600 focus:border-blue-500 outline-none"
                />
                <Button
                  type="button"
                  size="sm"
            ...
            <div>
              <label className="text-xs text-gray-600 dark:text-gray-400 mb-1 block">コート数</label>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 p-4 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">ブロック構成</h3>
            <span className="text-xs text-gray-600 dark:text-gray-400">{blocks.length}ブロック / {totalTeamsFromBlocks}チーム</span>
          </div>
          <div className="flex flex-wrap gap-2">
            {blocks.map((block, i) => (
              <div key={i} className="flex items-center gap-1 bg-gray-50 dark:bg-slate-800 rounded-lg px-3 py-2 border border-gray-200 dark:border-slate-600">
                <span className="text-sm font-bold text-blue-600 dark:text-blue-400">{block.label}</span>
                <select
                  value={block.size}
                  onChange={e => {
                  ...
                  className="bg-white dark:bg-slate-700 text-gray-800 dark:text-white text-sm rounded px-1.5 py-0.5 border border-gray-300 dark:border-slate-600 outline-none cursor-pointer"
                >
                  {[3, 4, 5, 6, 7, 8].map(n => (
                ...
                {blocks.length > 1 && (
                  <button
                    ...
                    className="ml-1 text-red-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300 transition-colors"
                  >
          ...
          {totalTeamsFromBlocks !== teamCount && (
            <p className="text-xs text-yellow-600 dark:text-yellow-400">⚠ チーム数合計が{totalTeamsFromBlocks}です（設定: {teamCount}）</p>
          )}
        </div>

        {/* Mode Selection */}
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 p-4 space-y-3">
          <h3 className="text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">生成モード</h3>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 p-4">
        <div className="flex items-center gap-3">
          ...
          {schedule && (
            <div className="ml-auto text-sm text-gray-700 dark:text-gray-300 space-y-1">
      ...
      {schedule && (
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 p-4">
          <h3 className="text-base font-bold text-gray-800 dark:text-gray-200 mb-3">生成されたスケジュール</h3>
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  {mode === "league" && (
                    <th className="px-3 py-3 text-left text-gray-700 dark:text-gray-300 text-sm font-semibold border-b-2 border-gray-300 dark:border-slate-600 w-16">節</th>
                  )}
                  <th className="px-3 py-3 text-left text-gray-700 dark:text-gray-300 text-sm font-semibold border-b-2 border-gray-300 dark:border-slate-600 w-16">#</th>
                  {Array.from({ length: courtCount }, (_, i) => (
                    <th
                      key={i}
                      className="px-3 py-3 text-center text-sm font-bold border-b-2 border-gray-300 dark:border-slate-600"
              ...
              <tbody>
                {schedule.map((slot, slotIdx) => (
                  <tr key={slotIdx} className="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                    {mode === "league" && slot.round && (
                      <td className="px-3 py-2.5 text-blue-600 dark:text-blue-400 font-semibold text-sm border-b border-gray-200 dark:border-slate-700">
                        {slot.round}
                      </td>
                    )}
                    <td className="px-3 py-2.5 text-gray-600 dark:text-gray-400 font-mono text-sm border-b border-gray-200 dark:border-slate-700">
                      {slotIdx + 1}
                    </td>
                    {slot.matches.map((match, courtIdx) => (
                      <td key={courtIdx} className="px-2 py-2.5 text-center border-b border-gray-200 dark:border-slate-700">
                        {match ? (
                          <span

    ...
    switch (action) {
      ...
      case "waitlist":
        return {
          ...
          commentLabel: "運営からのメッセージ（任意）",
          commentPlaceholder:
            "例: 参加枠の調整により、一時的にキャンセル待ちとさせていただきます。",
        };
      ...
      case "cancel":
        return {
          ...
          commentLabel: "運営からのメッセージ（任意）",
          commentPlaceholder:
            "例: 大変申し訳ございませんが、運営上の都合によりキャンセルとさせていただきます。",
        };
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px]">
        ...
        <div className="space-y-4 py-4">
          {/* 辞退申請の場合、辞退理由を表示 */}
          {(action === "approve_withdrawal" || action === "reject_withdrawal") &&
          ...
          {team.waitlist_position && (
            <div className="bg-gray-50 p-4 rounded-lg space-y-2 text-base">
              <div>
                <span className="font-semibold">キャンセル待ち順位:</span> {team.waitlist_position}
                位
              </div>
            </div>
          )}
          ...
          {config.showComment && (
            <div className="space-y-6">
              <Label htmlFor="admin-comment" className="text-base">
                {config.commentLabel}
              </Label>
              <Textarea
                id="admin-comment"
        ...
        <DialogFooter>
          ...
          <Button variant={config.confirmVariant} onClick={handleSubmit} disabled={submitting}>

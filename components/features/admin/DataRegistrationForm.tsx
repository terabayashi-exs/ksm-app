...
import { AlertTriangle, ArrowRight, CheckCircle, Search, RefreshCw, Users, UserPlus, Trash2 } from 'lucide-react';
...
function TeamReassignmentTab() {
  ...
  useEffect(() => {
    ...
    const fetchTeams = async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/admin/data-registration/tournament-teams?tournament_id=${selectedTournamentId}`);
        const data = await res.json();
        if (data.success) {
  ...
  const handleTeamSearch = async () => {
    ...
    setError('');
    try {
      const res = await fetch(`/api/admin/data-registration/search-teams?q=${encodeURIComponent(teamSearchQuery.trim())}`);
      const data = await res.json();
      if (data.success) {
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setSuccess(`チームIDを付け替えました: ${data.data.old_team_name} (${data.data.old_team_id}) → ${data.data.new_team_name} (${data.data.new_team_id})`);
        // リセット
        setSelectedTournamentTeamId('');
        setSelectedNewTeamId('');
        setSearchResults([]);
        ...
        // チーム一覧を再取得
        if (selectedTournamentId) {
          const refreshRes = await fetch(`/api/admin/data-registration/tournament-teams?tournament_id=${selectedTournamentId}`);
          ...
          {selectedTournamentId && (
            <div className="space-y-2">
              ...
              {loading && !tournamentTeams.length ? (
                ...
                <Select value={selectedTournamentTeamId} onValueChange={setSelectedTournamentTeamId}>
                  <SelectTrigger>
              ...
              {selectedTeam && (
                <div className="p-3 bg-gray-50 rounded-lg text-sm space-y-1">
                  <p><span className="font-medium">現在のチーム名:</span> {selectedTeam.team_name}</p>
                  <p><span className="font-medium">現在のteam_id:</span> {selectedTeam.team_id}</p>
                  <p><span className="font-medium">マスターチーム名:</span> {selectedTeam.master_team_name ?? '(不明)'}</p>
                  <p><span className="font-medium">参加ステータス:</span> <Badge variant="outline">{selectedTeam.participation_status}</Badge></p>
                </div>
              )}
            </div>
          )}
          ...
          {selectedTournamentTeamId && (
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                />
                <Button onClick={handleTeamSearch} disabled={loading || !teamSearchQuery.trim()} variant="outline">
...
function ForceRepresentativeTab() {
  ...
  const handleUserSearch = async () => {
    ...
    setUserInfo(null);
    try {
      const res = await fetch(`/api/admin/data-registration/search-user?email=${encodeURIComponent(emailInput.trim())}`);
      const data = await res.json();
      if (data.success) {
  ...
  const fetchTeamMembers = async (teamId: string) => {
    try {
      const res = await fetch(`/api/admin/data-registration/team-members?team_id=${encodeURIComponent(teamId)}`);
      const data = await res.json();
      if (data.success) {
  ...
  const canAddMember = teamMembers.length < 2 && userInfo && !teamMembers.some(m => m.login_user_id === userInfo.login_user_id);
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      if (data.success) {
        const d = data.data;
        setSuccess(`${d.display_name} (${d.email}) を ${d.team_name} の担当者に設定しました（${d.member_count}人目）`);
        // 担当者一覧とユーザー情報を再取得
        fetchTeamMembers(selectedTeamId);
        const refreshRes = await fetch(`/api/admin/data-registration/search-user?email=${encodeURIComponent(userInfo.email)}`);
        const refreshData = await refreshRes.json();
        if (refreshData.success) {
    ...
    <div className="space-y-6">
      <Card>
        ...
        <CardContent className="space-y-6">
          {/* Step 1: ユーザー検索 */}
          <div className="space-y-2">
            <label className="text-sm font-medium text-gray-700">1. ユーザーをメールアドレスで検索</label>
            <div className="flex gap-2">
              <Input
                type="email"
              ...
              />
              <Button onClick={handleUserSearch} disabled={loading || !emailInput.trim()} variant="outline">
                <Search className="h-4 w-4 mr-1" />
                検索
              </Button>
            </div>
            ...
            {userInfo && (
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg space-y-3">
                ...
                <div className="text-sm space-y-1">
                  <p><span className="font-medium">名前:</span> {userInfo.display_name}</p>
                  <p><span className="font-medium">メール:</span> {userInfo.email}</p>
                  <p><span className="font-medium">ID:</span> {userInfo.login_user_id}</p>
                  <p><span className="font-medium">ステータス:</span> {userInfo.is_active ? <Badge className="bg-green-100 text-green-700">有効</Badge> : <Badge className="bg-red-100 text-red-700">無効</Badge>}</p>
                </div>
                    ...
                    <div className="space-y-1">
                      {userInfo.memberships.map((m, i) => (
                        <div key={i} className="text-sm flex items-center gap-2">
                          <span>{m.team_name} (ID: {m.team_id})</span>
                          <Badge variant="outline" className="border-blue-400 text-blue-700">担当者</Badge>
                          {!m.is_active && <Badge className="bg-gray-100 text-gray-500">無効</Badge>}
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          {selectedTeamId && (
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700">
                3. 現在の担当者状況
                <Badge className="ml-2" variant="outline">{teamMembers.length} / 2 名</Badge>
              </label>

              {teamMembers.length === 0 ? (
                <p className="text-sm text-gray-500 p-3 bg-gray-50 rounded-lg">担当者が登録されていません</p>
              ) : (
                <div className="space-y-2">
                ...
                <Card className="border-amber-200 bg-amber-50">
                  <CardContent className="pt-4">
                    ...
                    <div className="text-sm space-y-2">
                      <p><span className="font-medium">ユーザー:</span> {userInfo?.display_name} ({userInfo?.email})</p>
                      <p><span className="font-medium">チーム:</span> {selectedTeam?.team_name}</p>
                      <p><span className="font-medium">登録:</span> {teamMembers.length === 0 ? '1人目の担当者' : '2人目の担当者'}として追加</p>
                    </div>
                    <Button
                      onClick={handleExecute}
                      disabled={executing}
              ...
              ) : userInfo && teamMembers.some(m => m.login_user_id === userInfo.login_user_id) ? (

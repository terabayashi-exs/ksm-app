...
export default function AdministratorManagement() {
  ...
  const handleUpdateUser = async () => {
    if (!userEditData.display_name.trim()) {
      setError("表示名を入力してください");
      return;
    }
    if (userEditData.password && userEditData.password.length < 6) {
      setError("パスワードは6文字以上で入力してください");
      return;
    }
  ...
  const handleDelete = async (admin: Administrator) => {
    ...
    if (
      !confirm(
        `管理者「${admin.admin_name}」の管理者権限を削除しますか？\n\n※アカウント自体は削除されません。管理者ロールのみ削除されます。`,
      )
    )
      return;
                ...
                <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 space-y-2">
                  ...
                  <div className="text-sm text-blue-700 space-y-1 ml-7">
                    <div>
                      ...
                      {emailCheckResult.user.display_name}
                    </div>
                    <div>
                      <span className="font-medium">メール：</span>
                      {emailCheckResult.user.email}
                    </div>
                  </div>
                </div>
                <p className="text-sm text-gray-500">
              ...
              <>
                <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 space-y-1">
                  <div className="flex items-center gap-2 text-amber-800 font-medium text-sm">
                    <User className="h-4 w-4" />「{emailInput}」のアカウントが見つかりませんでした
                  </div>
                  <p className="text-sm text-amber-700 ml-6">
                    新規アカウントを作成して管理者として登録します。
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) =>
                        setFormData((prev) => ({ ...prev, is_superadmin: e.target.checked }))
                      }
                    />
          ...
          <CardContent className="space-y-4">
            {userCreateStep === "email" && (
              <>
                ...
                <div>
                  <Label htmlFor="user_check_email">
                    メールアドレス <span className="text-destructive">*</span>
                  </Label>
                  <div className="flex gap-2 mt-1">
                    <Input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter") handleCheckUserEmail();
                      }}
                      placeholder="例: user@example.com"
                    />
                    <Button
                      onClick={handleCheckUserEmail}
                      disabled={checkingUserEmail}
                      className="shrink-0"
                    >
                      <Search className="h-4 w-4 mr-1" />
              ...
              <>
                <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 space-y-2">
                  ...
                  <div className="text-sm text-blue-700 space-y-1 ml-7">
                    <div>
                      <span className="font-medium">表示名：</span>
                      {userCheckResult.user.display_name}
                    </div>
                    <div>
                      <span className="font-medium">メール：</span>
                      {userCheckResult.user.email}
                    </div>
                  </div>
                </div>
                <p className="text-sm text-gray-500">
              ...
              <>
                <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 space-y-1">
                  <div className="flex items-center gap-2 text-blue-800 font-medium text-sm">
                    <Edit className="h-4 w-4" />「{userCheckResult.user.email}
                    」のアカウント情報を編集
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="user_edit_display_name">
                      表示名（氏名） <span className="text-destructive">*</span>
                    </Label>
                    <Input
                      ...
                      onChange={(e) =>
                        setUserEditData((prev) => ({ ...prev, display_name: e.target.value }))
                      }
                      placeholder="例: 田中太郎"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      id="user_edit_email_display"
                      value={userCheckResult.user.email}
                      disabled
                      className="bg-gray-50"
                    />
                  </div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setUserEditData((prev) => ({ ...prev, password: e.target.value }))
                    }
                    placeholder="6文字以上"
                  />
                  <p className="text-xs text-gray-500 mt-1">
              ...
              <>
                <div className="rounded-lg border border-green-200 bg-green-50 p-4 space-y-1">
                  <div className="flex items-center gap-2 text-green-800 font-medium text-sm">
                    <UserPlus className="h-4 w-4" />「{userEmailInput}」は未登録です
                  </div>
                  <p className="text-sm text-green-700 ml-6">新規アカウントを作成します。</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="user_display_name">
                      表示名（氏名） <span className="text-destructive">*</span>
                    </Label>
                    <Input
                ...
                <div className="space-y-4">
                  ...
                  {hasMoreAdmins && (
                    <div className="text-center pt-2">
                      ...
                      >
                        さらに表示する（残り {filteredAdministrators.length - adminDisplayCount}{" "}
                        件）
                      </Button>
                    </div>
                  )}
                    ...
                    >
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        ...
                        {getRoleBadges(user.roles).map((badge, i) => (
                          <span key={i} className={`px-2 py-1 text-xs rounded ${badge.className}`}>
                            {badge.label}
                          </span>
                        ))}
                      </div>

...
import { ChevronRight, Clock, Home, LogOut, Mail, UserCog } from "lucide-react";
...
export default function TeamManagersClient({
  teamId,
  teamName,
  teamOmission,
}: TeamManagersClientProps) {
    ...
    try {
      ...
      if (invitationsRes.ok) {
        ...
        if (invitationsData.success) {
          setInvitations(
            invitationsData.data.filter((inv: TeamInvitation) => inv.status === "pending"),
          );
        }
      }
    } catch (err) {
      ...
      <div className="bg-base-800 border-b-[3px] border-primary">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="py-6">
            ...
            <p className="text-sm text-white/70 mt-1">チームの代表者の招待・管理を行います</p>
          </div>
        </div>
      </div>
          ...
          <CardContent className="space-y-6">
            ...
            {loading ? (
              ...
              <>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {managers.map((m) => (
                      <div
                        key={m.login_user_id}
                        className="flex items-center justify-between gap-3 p-4 bg-gray-50/40 rounded-lg"
                      >
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      {invitations.map((inv) => (
                        <div
                          key={inv.id}
                          className="flex items-center justify-between p-4 bg-amber-50/60 rounded-lg border border-amber-200"
                        >
                ...
                {managers.length >= 2 && (
                  <p className="text-sm text-gray-500 pt-4 border-t">担当者は最大2名までです。</p>
                )}
              </>
            )}
          </CardContent>

...
import { ChevronRight, Home, Plus, Save, Trash2, Users } from "lucide-react";
...
export default function TeamPlayersClient({
  ...
  const handleSavePlayers = async () => {
    ...
    // 背番号の重複チェック
    const jerseyNumbers = validPlayers.map((p) => p.jersey_number.trim()).filter((n) => n !== "");
  ...
  const updatePlayer = (index: number, field: "player_name" | "jersey_number", value: string) => {
    setFormPlayers((prev) => prev.map((p, i) => (i === index ? { ...p, [field]: value } : p)));
  };
      ...
      <div className="bg-base-800 border-b-[3px] border-primary">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="py-6">
            ...
            <p className="text-sm text-white/70 mt-1">チームに所属する選手の登録・編集を行います</p>
          </div>
        </div>
      </div>
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm text-gray-500">（最大100名まで）</span>
                </div>

...
import { Search, Calendar, MapPin, Trophy, Loader2, X, Building2 } from 'lucide-react';
...
function getStatusBadgeVariant(status: TournamentStatus): 'muted' | 'info' | 'warning' | 'success' | 'error' {
  switch (status) {
...
export default function TournamentSearchSection({ sportTypes, initialTournaments, organizers }: TournamentSearchSectionProps) {
  ...
  return (
    <div>
      {/* ======== 検索ボックス ======== */}
      <div className="w-full max-w-2xl mx-auto rounded-2xl bg-white shadow-lg p-6 sm:p-8" id="search-box">
        {/* Search input */}
        <div className="flex gap-2">
          ...
          <Button
            ...
            className="shrink-0 rounded-lg px-5 group"
            size="lg"
          >
        ...
        {prefectures.length > 0 && (
          <div className="mt-4">
            ...
            <select
              ...
              onChange={(e) => handlePrefectureChange(e.target.value ? Number(e.target.value) : null)}
      ...
      <div className="mt-10 max-w-4xl mx-auto" id="tournament-results">
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-sm text-gray-500 mb-4">
              {hasSearched ? '検索条件を変更してお試しください' : '大会が公開されるとこちらに表示されます'}
            </p>
            {hasSearched && (
          ...
          <div className="space-y-5">
            {Array.from(grouped.entries()).map(([orgName, { logo_blob_url, tournaments: orgTournaments }]) => (
      ...
      {organizers.length > 0 && (
        <div className="mt-10 max-w-4xl mx-auto">
          <div className="text-center mb-8">
            <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">注目の主催者・公式リーグ</h2>
            <p className="text-sm text-gray-500">
              信頼できる協会・イベンターが開催する大会を探す
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {organizers.map((org) => (
              ...
              >
                <div className={`bg-white border-2 rounded-xl p-5 text-center cursor-pointer transition-all duration-200 hover:border-primary hover:-translate-y-1 hover:shadow-lg ${
                  activeOrganizerId === org.login_user_id ? 'border-primary shadow-lg' : 'border-gray-200'
                }`}>
                  ...
                  <p className="font-semibold text-sm text-gray-900 mb-1 line-clamp-2">{org.organization_name}</p>
                  <p className="text-xs text-gray-500">
                    {org.ongoing_count > 0 && <span className="text-green-600 font-medium">開催中: {org.ongoing_count}</span>}

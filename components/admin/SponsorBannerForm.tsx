...
export default function SponsorBannerForm({ tournamentId, banner, mode }: SponsorBannerFormProps) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_FILE_SIZE) {
      alert(
        `ファイルサイズが大きすぎます。最大${(MAX_FILE_SIZE / 1024 / 1024).toFixed(0)}MBまでです。`,
      );
      return;
    }
  ...
  const recommendedSizeText = getRecommendedSizeText(
    formData.banner_size,
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            {imageFile && <span className="ml-2 text-sm text-gray-500">{imageFile.name}</span>}
          </div>

          {imagePreview && (
            <div className="space-y-2">
              <div className="relative w-full h-64 bg-gray-100 rounded-lg overflow-hidden">
                <Image src={imagePreview} alt="プレビュー" fill className="object-contain" />
              </div>
              <p className="text-sm text-gray-500">バナー作成時に自動的にアップロードされます</p>
            </div>
          )}
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-4">
          <div>
            <Label htmlFor="banner_name">
              バナー名 <span className="text-destructive">*</span>
            </Label>
            <Input
              id="banner_name"
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            <Label htmlFor="banner_size">
              バナーサイズ <span className="text-destructive">*</span>
            </Label>
            <Select
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Label htmlFor="display_position">
                表示位置 <span className="text-destructive">*</span>
              </Label>
              <Select
                ...
                onValueChange={(value) =>
                  setFormData({
                    ...formData,
                    display_position:
                      value as (typeof BANNER_POSITIONS)[keyof typeof BANNER_POSITIONS],
                  })
                }
              >
                <SelectTrigger>
                ...
                <SelectContent>
                  {Object.values(BANNER_POSITIONS).map((position) => {
                    ...
                    return (
                      <SelectItem key={position} value={position} disabled={isDisabled}>
              ...
              {recommendedSizeText && (
                <p className="text-sm text-gray-500 mt-1">推奨サイズ: {recommendedSizeText}</p>
              )}
            ...
            <div>
              <Label htmlFor="target_tab">
                表示タブ <span className="text-destructive">*</span>
              </Label>
              <Select
                ...
                onValueChange={(value) =>
                  setFormData({
                    ...formData,
                    target_tab: value as (typeof TARGET_TABS)[keyof typeof TARGET_TABS],
                  })
                }
              >
                <SelectTrigger>
            ...
            <div>
              ...
              />
              <p className="text-sm text-gray-500 mt-1">数値が小さいほど先に表示されます</p>
            </div>
      ...
      <div className="flex gap-4">
        <Button type="submit" variant="outline" disabled={submitting}>
          {submitting ? "保存中..." : mode === "create" ? "バナーを作成" : "バナーを更新"}
        </Button>
        <Button
          type="button"
          variant="outline"

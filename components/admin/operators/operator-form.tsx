...
import type { OperatorFormData, TournamentAccessConfig } from "@/lib/types/operator";
...
export default function OperatorForm({
  ...
  const onSubmit = async (data: Record<string, string>) => {
    ...
    try {
      ...
      const allTournamentAccess =
        mode === "edit" && groupId ? [...tournamentAccess, ...otherGroupsAccess] : tournamentAccess;
      ...
      const url = mode === "create" ? "/api/admin/operators" : `/api/admin/operators/${operatorId}`;
      ...
      if (mode === "create") {
        ...
        // 編集の場合は通常のアラート
        alert("運営者情報を更新しました");
        const url = groupId ? `/admin/operators?group_id=${groupId}` : "/admin/operators";
        router.push(url);
        router.refresh();
      }
    } catch (error) {
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input id="operatorName" {...register("operatorName")} placeholder="山田 太郎" />
            {errors.operatorName && (

...
async function getExistingTournamentPlayersForSpecificTeam(
  tournamentId: number,
  teamId: string,
  tournamentTeamId: number,
): Promise<ExistingTournamentPlayer[]> {
...
export default async function TournamentJoinPage({ params, searchParams }: PageProps) {
  ...
  if (!session || session.user.role !== "team") {
    redirect(
      `/auth/login?callbackUrl=${encodeURIComponent(`/tournaments/${tournamentId}/entry/join`)}`,
    );
  }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            {actualEditMode
              ? specificTeamId
                ? `「${existingTournamentTeamInfo?.team_name || "チーム"}」の参加選手変更`
                : "参加選手の変更"
              : isNewTeamMode
                ? "参加チームを追加"
                : "大会参加申し込み"}
          ...
          {actualEditMode && specificTeamId && (
            <div className="mt-2 p-3 bg-orange-50 border border-orange-200 rounded-md">
              <p className="text-sm text-orange-800">
                <strong>個別チーム編集:</strong> 「{existingTournamentTeamInfo?.team_name}(
                {existingTournamentTeamInfo?.team_omission})」チームの選手情報を編集しています。
              </p>
            </div>
          )}
          {isNewTeamMode && (
            <div className="mt-2 p-3 bg-green-50 border border-green-200 rounded-md">
              <p className="text-sm text-green-800">
                <strong>複数チーム参加モード:</strong>{" "}
                同じマスターチームから追加のチームで参加します。異なるチーム名・略称を入力してください。
              </p>
            </div>
          )}
        </div>

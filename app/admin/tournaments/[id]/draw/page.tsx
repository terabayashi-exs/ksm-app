...
import { ChevronDown, ChevronRight, ChevronUp, Home, RotateCcw, Save, Shuffle } from "lucide-react";
...
export default function TournamentDrawPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (tData.phases?.phases?.length) {
          const sorted = [...tData.phases.phases].sort(
        ...
        const formattedTeams = teams
          .filter((team: unknown): team is SimpleTournamentTeam => {
            return (
              ...
              typeof team === "object" &&
              "team_id" in team &&
              Boolean((team as SimpleTournamentTeam).team_id)
            );
        ...
        const matchesResponse = await fetch(
          `/api/tournaments/${tournamentId}/matches?includeBye=true`,
        );
        ...
        console.log(
          "[Draw] 最初の3試合のデータ:",
          allMatches.slice(0, 3).map((m: Match) => ({
        ...
        try {
          ...
          if (assignmentsResponse.ok && assignmentsData.success) {
            const teams = Array.isArray(assignmentsData.data)
              ? assignmentsData.data
              : assignmentsData.data.teams || [];
            teams.forEach(
              (team: { assigned_block?: string; block_position?: number; team_name: string }) => {
        ...
        allMatches.forEach((m: Match) => {
          console.log(
            `[Draw] 試合 ${m.match_code}: is_bye_match=${m.is_bye_match} (type: ${typeof m.is_bye_match})`,
          );
          if (m.is_bye_match === 1) {
        ...
        const processedMatches = allMatches.map((m: Match, idx: number) => {
          ...
          if (m.team1_source && byeMatchWinners[m.team1_source]) {
            console.log(
              `[Draw] Match ${m.match_code}: team1_source=${m.team1_source} → ${byeMatchWinners[m.team1_source]}`,
            );
            resolvedTeam1DisplayName = byeMatchWinners[m.team1_source];
          }
          if (m.team2_source && byeMatchWinners[m.team2_source]) {
            console.log(
              `[Draw] Match ${m.match_code}: team2_source=${m.team2_source} → ${byeMatchWinners[m.team2_source]}`,
            );
          ...
          const newTeam1Name =
            m.is_bye_match === 1 && resolvedTeam1DisplayName !== m.team1_display_name
              ? resolvedTeam1DisplayName
              : !m.team1_tournament_team_id
                ? resolvedTeam1DisplayName !== m.team1_display_name
                  ? resolvedTeam1DisplayName
                  : m.team1_name
                : m.team1_name;
          const newTeam2Name =
            m.is_bye_match === 1 && resolvedTeam2DisplayName !== m.team2_display_name
              ? resolvedTeam2DisplayName
              : !m.team2_tournament_team_id
                ? resolvedTeam2DisplayName !== m.team2_display_name
                  ? resolvedTeam2DisplayName
                  : m.team2_name
                : m.team2_name;
        ...
        // ブロック別の想定チーム数を取得
        const blockCountsResponse = await fetch(
          `/api/tournaments/${tournamentId}/block-team-counts`,
        );
  ...
  const initializeBlocks = useCallback(
    async (_teams: Team[], matches: Match[], firstPhaseId: string = "preliminary") => {
      // 最初のフェーズのブロックを抽出
      ...
      try {
        ...
        if (response.ok && teamsData.success) {
          ...
          // 各ブロックの最大チーム数を動的に計算
          const maxTeamsPerBlock = Math.max(
            6,
            Math.ceil(assignedTeams.length / preliminaryBlocks.size),
          );
          ...
          assignedTeams.forEach(
            (team: {
              ...
              if (
                team.assigned_block &&
                team.block_position &&
                preliminaryBlocks.has(team.assigned_block) &&
                team.team_id &&
                team.team_name
              ) {
          ...
          const initialBlocks: Block[] = Array.from(preliminaryBlocks)
            .sort()
            .map((blockName) => ({
  ...
  const getFirstRoundSlots = (blockName: string): FirstRoundSlot[] => {
    ...
    console.log(`[getFirstRoundSlots] Block ${blockName}:`, {
      ...
      preliminaryMatches: matches.filter(
        (m) => m.phase === getFirstPhaseId() && m.block_name === blockName,
      ).length,
      ...
      if (initialMatches.length > 0) {
        ...
        resetMatches.forEach((m) => {
          if (m.team1_source && byeMatchWinners[m.team1_source]) {
            ...
            console.log(
              `[Reset] ${m.match_code}: team1_source=${m.team1_source} → ${m.team1_display_name}`,
            );
          }
          if (m.team2_source && byeMatchWinners[m.team2_source]) {
            m.team2_display_name = byeMatchWinners[m.team2_source];
            console.log(
              `[Reset] ${m.match_code}: team2_source=${m.team2_source} → ${m.team2_display_name}`,
            );
          }
        });
    ...
    if (isTournament) {
      ...
      newMatches.forEach((m) => {
        if (m.team1_source && byeMatchWinners[m.team1_source]) {
          ...
          console.log(
            `[RandomDraw] ${m.match_code}: team1_source=${m.team1_source} → ${m.team1_name}`,
          );
        }
        if (m.team2_source && byeMatchWinners[m.team2_source]) {
          m.team2_name = byeMatchWinners[m.team2_source];
          console.log(
            `[RandomDraw] ${m.match_code}: team2_source=${m.team2_source} → ${m.team2_name}`,
          );
        }
      });
      ...
      if (blockTeamCounts.length > 0) {
        ...
        blocks.forEach((block, blockIndex) => {
          ...
          const count =
            expectedCount !== null
              ? expectedCount
              : Math.ceil(shuffledTeams.length / blocks.length);
  ...
  const moveTeam = (tournamentTeamId: number, fromBlockIndex: number, toBlockIndex: number) => {
    ...
    const teamIndex = fromBlock.teams.findIndex(
      (team) => team && team.tournament_team_id === tournamentTeamId,
    );
  ...
  // トーナメント形式: チーム単位で移動
  const handleTournamentMoveTeam =
    ...
    (
      ...
      // 移動元のチーム情報を取得
      const fromTeamId =
        fromTeamPosition === "team1"
          ? fromMatch.team1_tournament_team_id
          : fromMatch.team2_tournament_team_id;
      const fromTeamName =
        fromTeamPosition === "team1" ? fromMatch.team1_name : fromMatch.team2_name;

      // 移動先のチーム情報を取得
      const toTeamId =
        toTeamPosition === "team1"
          ? toMatch.team1_tournament_team_id
          : toMatch.team2_tournament_team_id;
      ...
      if (blockTeamCounts.length > 0) {
        ...
        blocks.forEach((block) => {
          ...
          if (expectedCount !== null && currentCount !== expectedCount) {
            errors.push(
              `${block.block_name}ブロック: ${currentCount}/${expectedCount}チーム (想定と一致しません)`,
            );
          }
        });
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          >
            <CardTitle className="flex items-center justify-between">
              <div className="flex flex-col gap-2">
                ...
                {!isTeamListExpanded && (
                  <span className="text-sm font-normal text-gray-500">クリックして展開</span>
                )}
              </div>
            </CardTitle>
          </CardHeader>
          {isTeamListExpanded && (
            <CardContent>
              ...
              {registeredTeams.length === 0 ? (
                <p className="text-gray-500 text-center py-8">参加チームがありません</p>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {registeredTeams.map((team) => {
                    ...
                    const isAssigned = blocks.some(
                      (block) =>
                        block.teams &&
                        block.teams.some(
                          (blockTeam) =>
                            blockTeam && blockTeam.tournament_team_id === team.tournament_team_id,
                        ),
                    );
                        ...
                        <div className="flex justify-between items-center">
                          <div>
                            <p className={`font-medium ${isAssigned ? "text-gray-500" : ""}`}>
                              {team.team_name}
                              {isAssigned && (
                                <span className="text-xs text-green-600 ml-2">(振分け済み)</span>
                              )}
                            </p>
                            <p
                              className={`text-sm ${isAssigned ? "text-gray-500" : "text-gray-500"}`}
                            >
                              {team.team_omission || ""}
                            </p>
                          </div>
                ...
                <Card key={block.block_name} className="h-full flex flex-col">
                  <CardHeader className={headerClass}>
                    <CardTitle className="text-lg">
                      {block.block_name}ブロック ({currentCount}
                      {expectedCount !== null ? `/${expectedCount}` : ""}チーム)
                    </CardTitle>
                  </CardHeader>
                              ...
                              <div className="flex justify-between items-center">
                                ...
                                <div className="flex items-center space-x-2">
                                  ...
                                  <Button
                                    ...
                                    variant="outline"
                                    onClick={() =>
                                      moveTeamWithinBlock(blockIndex, teamIndex, "down")
                                    }
        ...
        {matches.length > 0 &&
          (() => {
            const firstPhaseMatches = matches.filter(
              (m) => m.is_bye_match !== 1 && m.phase === getFirstPhaseId(),
            );
            ...
            return (
              <Card className="mt-8">
                <CardHeader>
                  ...
                  <p className="text-sm text-gray-500">チーム振分後の試合対戦表です</p>
                </CardHeader>
                <CardContent>
                    ...
                    <table className="w-full table-auto">
                      ...
                      <tbody>
                        {firstPhaseMatches.map((match, index, filteredMatches) => {
                          ...
                          if (hasMatchday) {
                            // 節ごとにグルーピング
                            const showMatchdayHeader =
                              index === 0 || filteredMatches[index - 1].matchday !== match.matchday;
                            ...
                            const showRoundHeader =
                              index === 0 ||
                              filteredMatches[index - 1].round_name !== match.round_name;
                            if (showRoundHeader) {
                              rows.push(
                            ...
                            >
                              ...
                              <td className="px-4 py-3">
                                <div className="flex items-center space-x-2">
                                  ...
                                  >
                                    {match.team1_name ||
                                      resolveTeamSource(match.team1_source) ||
                                  ...
                                  >
                                    {match.team2_name ||
                                      resolveTeamSource(match.team2_source) ||
                              ...
                              <td className="px-4 py-3 text-sm">
                                <div>
                                  <p>
                                    {new Date(match.tournament_date).toLocaleDateString("ja-JP")}
                                  </p>
                                  {match.start_time && (
                              ...
                              {hasMatchday ? (
                                <>
                                  <td className="px-4 py-3 text-sm">{match.venue_name || "-"}</td>
                                  <td className="px-4 py-3 text-sm">

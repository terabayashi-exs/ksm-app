...
export default async function EditOperatorPage({ params, searchParams }: PageProps) {
  ...
  const groupId = resolvedSearchParams.group_id
    ? parseInt(resolvedSearchParams.group_id, 10)
    : undefined;
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="container mx-auto py-8 px-4 max-w-4xl">
        <nav className="flex flex-wrap items-center gap-1.5 text-sm mb-6">
          ...
          <ChevronRight className="h-4 w-4 text-gray-400 flex-shrink-0" />
          <Link
            href={groupId ? `/admin/operators?group_id=${groupId}` : "/admin/operators"}
            className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-md bg-gray-100 text-gray-600 hover:bg-gray-200 hover:text-gray-900 transition-colors whitespace-nowrap"
          >
            運営者管理
          </Link>
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">部門アクセス権と操作権限を変更できます</p>
        </div>
        <EditOperatorForm

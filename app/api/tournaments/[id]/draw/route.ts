...
async function autoConfirmWalkoverMatches(tournamentId: number): Promise<void> {
  try {
    console.log(
      `[AUTO_CONFIRM_WALKOVER] Checking for walkover matches in tournament ${tournamentId}`,
    );
    ...
    console.log(
      `[AUTO_CONFIRM_WALKOVER] Found ${walkoverMatchesResult.rows.length} walkover matches to auto-confirm`,
    );
    ...
    for (const match of walkoverMatchesResult.rows) {
      ...
      console.log(
        `[AUTO_CONFIRM_WALKOVER] ✅ Auto-confirmed walkover match ${matchCode} (ID: ${matchId}), winner_tt_id: ${winnerTournamentTeamId}`,
      );
      ...
      try {
        ...
        console.log(
          `[AUTO_CONFIRM_WALKOVER] ✅ Processed progression for walkover match ${matchCode}`,
        );
      } catch (progressionError) {
        console.error(
          `[AUTO_CONFIRM_WALKOVER] Failed to process progression for ${matchCode}:`,
          progressionError,
        );
      }
    }
    ...
    console.log(
      `[AUTO_CONFIRM_WALKOVER] ✅ Completed auto-confirmation of ${walkoverMatchesResult.rows.length} walkover matches`,
    );
  } catch (error) {
    ...
    try {
      ...
      for (const block of blocks) {
        ...
        for (const team of block.teams) {
          ...
          if (usedPositions.has(team.block_position)) {
            console.error(
              `[Draw Save API] ⚠️ Duplicate block_position detected: ${block.block_name} position ${team.block_position}`,
            );
            return NextResponse.json(
              {
                success: false,
          ...
          `,
            [block.block_name, team.block_position, team.tournament_team_id],
          );
        }
      }
      ...
      if (frontendMatches && Array.isArray(frontendMatches) && frontendMatches.length > 0) {
        ...
        for (const match of frontendMatches) {
          console.log(
            `[Draw Save API] Updating match ${match.match_id}: team1_tt_id=${match.team1_tournament_team_id}, team2_tt_id=${match.team2_tournament_team_id}`,
          );
          ...
          if (match.team1_tournament_team_id && match.team2_tournament_team_id) {
            await db.execute(
              ...
              WHERE match_id = ?
            `,
              [match.team1_tournament_team_id, match.team2_tournament_team_id, match.match_id],
            );
          }
          // BYE試合：team1のみ
          // MIGRATION NOTE: team_id系フィールドは設定しない（tournament_team_idのみ使用）
            ...
            await db.execute(
              ...
              WHERE match_id = ?
            `,
              [match.team1_tournament_team_id, match.match_id],
            );
          }
          // BYE試合：team2のみ
          // MIGRATION NOTE: team_id系フィールドは設定しない（tournament_team_idのみ使用）
            ...
            await db.execute(
              ...
              WHERE match_id = ?
            `,
              [match.team2_tournament_team_id, match.match_id],
            );
          }
        }
      }
      ...
      for (const block of blocks) {
        block.teams.forEach(
          (team: { tournament_team_id: number; block_position: number }, index: number) => {
      ...
      for (const match of allMatchesResult.rows) {
        ...
        if (team1Pos !== null && team2Pos !== null) {
          ...
          if (team1TournamentTeamId && team2TournamentTeamId) {
            ...
            console.log(
              `[Draw Save API] Updated match ${matchId}: ${team1Display}(tt_id:${team1TournamentTeamId}) vs ${team2Display}(tt_id:${team2TournamentTeamId})`,
            );
          }
        }
      }

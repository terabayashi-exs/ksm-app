...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const matches = matchesResult.rows.map((r) => ({
      ...
      team1_display_name: r.team1_omission
        ? String(r.team1_omission)
        : String(r.team1_display_name),
      team2_display_name: r.team2_omission
        ? String(r.team2_omission)
        : String(r.team2_display_name),
  ...
  try {
    ...
    if (dateCourtSettings && Array.isArray(dateCourtSettings)) {
      for (const dcs of dateCourtSettings) {
        ...
        if (dcs.venue_id) {
          const venueResult = await db.execute(
            `SELECT venue_name FROM m_venues WHERE venue_id = ?`,
            [dcs.venue_id],
          );

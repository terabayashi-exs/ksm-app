...
export async function PUT(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (!visibility || typeof visibility !== 'object') {
      return NextResponse.json({ success: false, error: 'visibility設定が必要です' }, { status: 400 });
    }
    ...
    try {
      phases = typeof phasesRaw === 'string' ? JSON.parse(phasesRaw) : phasesRaw as unknown as TournamentPhases;
    } catch {
      return NextResponse.json({ success: false, error: 'フェーズ情報の解析に失敗しました' }, { status: 500 });
    }

    if (!phases?.phases) {
      return NextResponse.json({ success: false, error: 'フェーズ情報がありません' }, { status: 400 });
    }

    // is_visible を更新
    phases.phases = phases.phases.map(phase => ({
      ...phase,
      is_visible: visibility[phase.id] !== undefined ? visibility[phase.id] : (phase.is_visible !== false)
    }));
    ...
    return NextResponse.json({
      ...
      phases: phases.phases.map(p => ({ id: p.id, name: p.name, is_visible: p.is_visible !== false }))
    });

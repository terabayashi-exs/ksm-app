...
export async function GET(request: NextRequest) {
  try {
    ...
    const rawStatsMap = new Map<
      number,
      Array<{ phase: string; block_count: number; max_court_number: number | null }>
    >();
    ...
    // 推奨ロジック
    const recommendedFormats: Array<
      (typeof annotatedFormats)[number] & {
    ...
    // 1. 完全一致（全て取得）
    const exactMatches = annotatedFormats.filter(
      (format) => format.target_team_count === teamCount,
    );

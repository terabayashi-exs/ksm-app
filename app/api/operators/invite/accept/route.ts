...
export async function GET(request: NextRequest) {
  ...
  if (String(invite.status) !== 'pending') {
    const errorMessage = String(invite.status) === 'accepted'
      ? 'この招待はすでに承認済みです'
  ...
  if (expiresAt < now) {
    ...
    return NextResponse.json({ success: false, error: 'この招待の有効期限が切れています' }, { status: 400 });
  }
  ...
  try {
    ...
    if (userResult.rows.length > 0) {
      ...
      if (existingEmail.toLowerCase() !== email.toLowerCase()) {
        ...
        return NextResponse.json({ success: false, error: 'メールアドレスが一致しません' }, { status: 400 });
      }
    } else {
      // 新規アカウント → アカウント作成
      ...
      try {
        ...
        if (existingAccess.rows.length === 0) {
          await db.execute({
            ...
            args: [loginUserId, access.tournamentId, JSON.stringify(access.permissions), invitedByLoginUserId]
          });
          console.log(`部門アクセス権を追加: operator_id=${loginUserId}, tournament_id=${access.tournamentId}, assigned_by=${invitedByLoginUserId}`);
        } else {
          // 既に存在する場合は権限を更新
          await db.execute({
            ...
            args: [JSON.stringify(access.permissions), invitedByLoginUserId, loginUserId, access.tournamentId]
          });
          console.log(`部門アクセス権を更新: operator_id=${loginUserId}, tournament_id=${access.tournamentId}, assigned_by=${invitedByLoginUserId}`);
        }
      } catch (accessError) {

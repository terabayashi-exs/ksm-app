...
async function checkTeamNameDuplication(
  ...
  `,
    currentTournamentTeamId
      ? [tournamentId, teamName, teamOmission, currentTournamentTeamId]
...
async function checkPlayerDuplication(
  ...
  `,
    currentTournamentTeamId
      ? [tournamentId, ...playerIds, currentTournamentTeamId]
  ...
  try {
    ...
    if (actualEditMode) {
      ...
      const participationStatus =
        maxTeams !== null && confirmedCount >= maxTeams ? "waitlisted" : "confirmed";
        ...
        if (teamMembersResult.rows.length > 0) {
          ...
          try {
            if (tournament.tournament_dates) {
              const datesData =
                typeof tournament.tournament_dates === "string"

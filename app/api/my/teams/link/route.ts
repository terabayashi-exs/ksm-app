...
export async function POST(request: NextRequest) {
  ...
  if (!loginUserId || loginUserId === 0) {
    return NextResponse.json({ success: false, error: 'ログインユーザー情報が取得できません' }, { status: 400 });
  }
  ...
  try {
    ...
    const team = teamResult.rows[0] as unknown as { team_id: string; team_name: string; team_omission: string };
    ...
    if (existingMemberResult.rows.length > 0) {
      const existingMember = existingMemberResult.rows[0] as unknown as { login_user_id: number; display_name: string };

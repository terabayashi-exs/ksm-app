...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!mainTeamId || !absorbedTeamId) {
      return NextResponse.json({ success: false, error: 'メインチームIDと統合対象チームIDが必要です' }, { status: 400 });
    }

    if (mainTeamId === absorbedTeamId) {
      return NextResponse.json({ success: false, error: '同じチームを指定することはできません' }, { status: 400 });
    }
    ...
    if (!memberTeamIds.includes(mainTeamId) || !memberTeamIds.includes(absorbedTeamId)) {
      return NextResponse.json({ success: false, error: '両方のチームの担当者である必要があります' }, { status: 403 });
    }
    ...
    return NextResponse.json({ success: false, error: 'チーム統合に失敗しました' }, { status: 500 });
  }
}

...
export async function GET(request: NextRequest) {
  try {
    ...
    if (String(invite.status) !== 'pending') {
      return NextResponse.json({ success: false, error: 'この招待は既に処理済みです' }, { status: 400 });
    }

    // 有効期限チェック
    if (invite.expires_at && new Date(String(invite.expires_at)) < new Date()) {
      return NextResponse.json({ success: false, error: 'この招待リンクは有効期限切れです' }, { status: 400 });
    }
    ...
    return NextResponse.json({ success: false, error: '招待情報の取得に失敗しました' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (inviteResult.rows.length === 0) {
      return NextResponse.json({ success: false, error: '無効または処理済みの招待です' }, { status: 400 });
    }
    ...
    if (userResult.rows.length > 0) {
      ...
      if (!displayName || !displayName.trim()) {
        return NextResponse.json({ success: false, error: '表示名を入力してください' }, { status: 400 });
      }
    ...
    if (existingMember.rows.length > 0) {
      ...
      return NextResponse.json({ success: true, message: '既にこのチームの担当者として登録されています' });
    }
    ...
    return NextResponse.json({ success: false, error: '担当者登録に失敗しました' }, { status: 500 });
  }
}

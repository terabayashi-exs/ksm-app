...
export async function POST(request: NextRequest) {
  ...
  if (!team_id || !invited_email) {
    return NextResponse.json(
      { success: false, error: "team_id と invited_email が必要です" },
      { status: 400 },
    );
  }
  ...
  // 招待先がアカウント保有者かチェック
  const userCheck = await db.execute(`SELECT login_user_id FROM m_login_users WHERE email = ?`, [
    invited_email,
  ]);
    ...
    return NextResponse.json(
      {
        success: false,
        error:
          "このメールアドレスはアカウント登録されていません。招待を送信するには、相手が先に大会GOにアカウント登録している必要があります。",
      },
      { status: 400 },
    );
  }
  ...
  if (existingMember.rows.length > 0) {
    return NextResponse.json(
      { success: false, error: "このメールアドレスはすでに担当者として登録されています" },
      { status: 400 },
    );
  }
    ...
    return NextResponse.json(
      {
        success: false,
        error:
          "このメールアドレスは既に別のチームの担当者として登録されています。1つのアカウントで複数のチームを管理することはできません。",
      },
      { status: 400 },
    );
  }
  ...
  if (existingInvite.rows.length > 0) {
    return NextResponse.json(
      { success: false, error: "このメールアドレスにはすでに招待を送信しています" },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { success: false, error: "招待メールの送信に失敗しました" },
      { status: 500 },
    );
  }
...
export async function DELETE(request: NextRequest) {
  ...
  if (!invitation_id) {
    return NextResponse.json(
      { success: false, error: "invitation_id が必要です" },
      { status: 400 },
    );
  }
  ...
  await db.execute(`UPDATE t_team_invitations SET status = 'cancelled' WHERE id = ?`, [
    invitation_id,
  ]);

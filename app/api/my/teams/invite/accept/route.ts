...
export async function GET(request: NextRequest) {
  ...
  if (String(invite.status) !== 'pending') {
    return NextResponse.json({
      success: false,
      error: String(invite.status) === 'accepted' ? 'この招待はすでに承認済みです' : 'この招待はキャンセルされています',
    }, { status: 400 });
  }
...
export async function POST(request: NextRequest) {
  ...
  if (!session?.user?.loginUserId || session.user.loginUserId === 0) {
    return NextResponse.json({ success: false, error: '認証が必要です。先にログインしてください。' }, { status: 401 });
  }
  ...
  if (existingMember.rows.length > 0) {
    return NextResponse.json({ success: false, error: 'すでにこのチームの担当者として登録されています' }, { status: 400 });
  }
  ...
  if (Number(otherTeam.rows[0]?.cnt ?? 0) > 0) {
    return NextResponse.json({
      success: false,
      error: '既に別のチームの担当者として登録されています。1つのアカウントで複数のチームを管理することはできません。'
    }, { status: 400 });
  }

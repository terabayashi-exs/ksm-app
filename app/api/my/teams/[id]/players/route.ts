...
export async function PUT(request: NextRequest, context: RouteContext) {
  ...
  let body: {
    players: { player_id?: number; player_name: string; jersey_number?: number | null }[];
  };
  try {
  ...
  if (players.length > 100) {
    return NextResponse.json(
      { success: false, error: "選手は100名以内にしてください" },
      { status: 400 },
    );
  }
  ...
  const deactivatedCount =
    currentIds.size - submittedIds.size > 0 ? currentIds.size - submittedIds.size : 0;

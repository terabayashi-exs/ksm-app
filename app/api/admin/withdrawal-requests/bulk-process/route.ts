...
export async function POST(request: NextRequest) {
  try {
    ...
    const { action, tournament_team_ids, admin_comment, individual_comments }: BulkProcessRequest =
      await request.json();
    ...
    const invalidStatus = withdrawalCheck.rows.filter(
      (row) => row.withdrawal_status !== "withdrawal_requested",
    );
    ...
    if (invalidStatus.length > 0) {
      const invalidTeams = invalidStatus.map(
        (row) => `${row.team_name} (ID: ${row.tournament_team_id})`,
      );
      return NextResponse.json(
        {
    ...
    return NextResponse.json({ error: "一括処理中にエラーが発生しました" }, { status: 500 });
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const tournament_team_ids = idsParam
      .split(",")
    ...
    const totalAffectedMatches = teamsInfo.rows.reduce(
      (sum, row) => sum + Number(row.affected_matches || 0),
      0,
    );
    const tournamentIds = [...new Set(teamsInfo.rows.map((row) => Number(row.tournament_id)))];
    const blocksAffected = [
      ...new Set(teamsInfo.rows.map((row) => row.assigned_block).filter(Boolean)),
    ];
    ...
    if (totalAffectedMatches > 20) {
      analysisResult.warnings.push(
        `${totalAffectedMatches}試合に影響します。処理に時間がかかる場合があります。`,
      );
    }

    if (action === "approve" && blocksAffected.length > 3) {
      analysisResult.warnings.push(
        `${blocksAffected.length}個のブロックに影響し、順位表の大幅な変更が発生します。`,
      );
    }

    if (teamsInfo.rows.some((row) => row.withdrawal_status !== "withdrawal_requested")) {
      analysisResult.warnings.push(
        "申請中以外のチームが含まれています。これらはスキップされます。",
      );
    }
    ...
    return NextResponse.json({ error: "影響分析中にエラーが発生しました" }, { status: 500 });
  }
}

...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // 同じブロックの他のチーム情報を取得
    const blockTeams = withdrawal.assigned_block
      ? await db.execute(
          `
      SELECT 
        tt.team_name,
    ...
    `,
          [withdrawal.tournament_id, withdrawal.assigned_block, withdrawal.team_id],
        )
      : { rows: [] };
    ...
    if (impact.blockAdjustment) {
      const affectedPositions = blockTeams.rows.filter(
        (team) => Number(team.block_position) > Number(withdrawal.block_position),
      ).length;
    ...
    return NextResponse.json({ error: "辞退影響分析中にエラーが発生しました" }, { status: 500 });
  }
}

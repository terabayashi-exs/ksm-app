  ...
  try {
    ...
    if (send_notification) {
      try {
        if (action === "approve") {
          // 承認通知を送信（バックグラウンドで実行）
          sendWithdrawalApprovedNotification(tournamentTeamId, admin_comment, adminId).catch(
            (error) => {
          ...
          // 却下通知を送信（バックグラウンドで実行）
          sendWithdrawalRejectedNotification(tournamentTeamId, admin_comment, adminId).catch(
            (error) => {
    ...
    return NextResponse.json({
      success: true,
      message: action === "approve" ? "辞退申請を承認しました" : "辞退申請を却下しました",
      data: {
        ...
        withdrawal_admin_comment: result.withdrawal_admin_comment
          ? String(result.withdrawal_admin_comment)
          : null,
    ...
    return NextResponse.json({ error: "辞退申請の処理中にエラーが発生しました" }, { status: 500 });
  }
}
  ...
  try {
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        withdrawal_requested_at: detail.withdrawal_requested_at
          ? String(detail.withdrawal_requested_at)
          : null,
        withdrawal_processed_at: detail.withdrawal_processed_at
          ? String(detail.withdrawal_processed_at)
          : null,
        withdrawal_processed_by: detail.withdrawal_processed_by
          ? String(detail.withdrawal_processed_by)
          : null,
        ...
        recruitment_end_date: detail.recruitment_end_date
          ? String(detail.recruitment_end_date)
          : null,

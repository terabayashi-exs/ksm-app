...
export async function GET() {
  ...
  const groups = await Promise.all(
    groupsResult.rows.map(async (row) => {
...
export async function POST(request: NextRequest) {
  ...
  if (!group_id || !new_owner_login_user_id) {
    return NextResponse.json(
      { success: false, error: "group_id と new_owner_login_user_id が必要です" },
      { status: 400 },
    );
  }
  ...
  if (groupResult.rows.length === 0) {
    return NextResponse.json(
      { success: false, error: "大会グループが見つかりません" },
      { status: 404 },
    );
  }
  ...
  if (newOwnerResult.rows.length === 0) {
    return NextResponse.json(
      { success: false, error: "指定されたユーザーが見つからないか、adminロールがありません" },
      { status: 400 },
    );
  }
  ...
  if (oldOwnerLoginUserId === Number(new_owner_login_user_id)) {
    return NextResponse.json(
      { success: false, error: "現在のオーナーと同じユーザーです" },
      { status: 400 },
    );
  }

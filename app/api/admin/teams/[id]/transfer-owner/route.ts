...
export async function PATCH(
  ...
  if (!new_primary_login_user_id) {
    return NextResponse.json({ success: false, error: 'new_primary_login_user_id が必要です' }, { status: 400 });
  }
  ...
  if (memberCheck.rows.length === 0) {
    return NextResponse.json({ success: false, error: '対象ユーザーはこのチームの担当者ではありません' }, { status: 400 });
  }
...
export async function DELETE(
  ...
  if (!login_user_id) {
    return NextResponse.json({ success: false, error: 'login_user_id が必要です' }, { status: 400 });
  }
  ...
  if (Number(countResult.rows[0].cnt) <= 1) {
    return NextResponse.json({ success: false, error: '最後の担当者は削除できません' }, { status: 400 });
  }

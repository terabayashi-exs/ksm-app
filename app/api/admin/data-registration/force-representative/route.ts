...
export async function POST(request: NextRequest) {
  try {
    ...
    const alreadyMember = currentMembers.rows.find(
      r => Number(r.login_user_id) === loginUserId
    );
    if (alreadyMember) {
      return NextResponse.json(
    ...
    if (currentMembers.rows.length >= 2) {
      return NextResponse.json(
        { success: false, error: 'このチームには既に2名の担当者が登録されています。追加するには先に既存の担当者を削除してください' },
        { status: 409 }
      );
    }
    ...
    if (roleCheck.rows.length === 0) {
      await db.execute(
        `INSERT INTO m_login_user_roles (login_user_id, role) VALUES (?, 'team')`,
        [loginUserId]
      );
    }

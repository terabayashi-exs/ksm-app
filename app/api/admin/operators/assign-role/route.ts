    ...
    for (const access of tournamentAccess) {
      try {
        ...
        if (existingAccess.rows.length === 0) {
          ...
          console.log(
            `部門アクセス権を追加: operator_id=${loginUserId}, tournament_id=${access.tournamentId}, assigned_by=${adminLoginUserId}`,
          );
        } else {
          ...
          console.log(
            `部門アクセス権を更新: operator_id=${loginUserId}, tournament_id=${access.tournamentId}, assigned_by=${adminLoginUserId}`,
          );
        }
      } catch (accessError) {
        console.error(
...
大会GO運営チーム
    ...
    return NextResponse.json({ error: "運営者権限の付与に失敗しました" }, { status: 500 });
  }
}

...
export async function POST(request: NextRequest) {
  try {
    ...
    if (new_group && !new_group.group_name?.trim()) {
      return NextResponse.json({ success: false, error: "新規大会名は必須です" }, { status: 400 });
    }
    ...
    if (new_group) {
      ...
      console.log(
        `[DUPLICATE] 新規大会グループ作成: ID=${targetGroupId}, 名前=${new_group.group_name}`,
      );
    ...
    console.log(
      `[DUPLICATE] 複製完了: 大会${source_tournament_id} -> 新部門${newTournamentId} (グループ${targetGroupId})`,
    );
 ...
 * ルール情報の複製
 */
async function duplicateTournamentRules(
  sourceTournamentId: number,

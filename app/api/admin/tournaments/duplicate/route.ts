...
async function duplicateTournament(
  ...
  // 1. 大会基本情報の複製
  const newTournamentId = await duplicateTournamentBasic(sourceTournamentId, newTournamentName);
  
  // 2. ルール情報の複製
  await duplicateTournamentRules(sourceTournamentId, newTournamentId);

  // 3. ブロック情報の複製（空の状態）
  const blockMapping = await duplicateMatchBlocks(sourceTournamentId, newTournamentId);
  ...
  // レベル4: 進行中データの複製
  if (level === 'level4') {
    await duplicateMatchProgress(sourceTournamentId, newTournamentId, matchMapping);
  }
 ...
 * 大会基本情報の複製
 */
async function duplicateTournamentBasic(sourceTournamentId: number, newTournamentName: string): Promise<number> {
  // 複製元の大会情報を取得
  ...
  const insertResult = await db.execute(`
    INSERT INTO t_tournaments (
      ...
      win_points, draw_points, loss_points, walkover_winner_goals, walkover_loser_goals,
      sport_type_id
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `, [
    newTournamentName,
    ...
    sourceData.win_points || 3,
    sourceData.draw_points || 1,
    sourceData.loss_points || 0,
    sourceData.walkover_winner_goals || 3,
    sourceData.walkover_loser_goals || 0,
 ...
 * ブロック情報の複製
 */
async function duplicateMatchBlocks(sourceTournamentId: number, newTournamentId: number): Promise<Map<number, number>> {
...
async function duplicateMatchProgress(
  ...
  // 試合の進行状況を複製
  const matchesResult = await db.execute(`
    SELECT ml.*, ms.* FROM t_matches_live ml
  ...
  for (const match of matchesResult.rows) {
    ...
    // t_match_status の複製（存在する場合）
    if (matchData.match_status) {
      await db.execute(`
        INSERT INTO t_match_status (
          match_id, match_status, current_period, updated_by, updated_at
        ) VALUES (?, ?, ?, ?, ?)
      `, [
        newMatchId,
        matchData.match_status,
        matchData.current_period || 1,
        matchData.updated_by || 'system',

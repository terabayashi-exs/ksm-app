...
export async function PUT(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (!confirmation) {
      return NextResponse.json({ success: false, error: "変更確認が必要です" }, { status: 400 });
    }
      ...
      return NextResponse.json(
        {
          ...
          details: {
            ...
            message:
              "大会のステータスが「計画中」または「募集中」の場合のみフォーマット変更が可能です。",
          },
        },
        { status: 400 },
      );
    ...
    const providedDays = Object.keys(existingTournamentDates)
      .map(Number)
      .sort((a, b) => a - b);
    ...
    for (let i = 1; i <= maxDayNumber; i++) {
      if (existingTournamentDates[i.toString()]) {
        ...
        // 不足している日を追加
        const previousDate =
          adjustedTournamentDates[(i - 1).toString()] ||
    ...
    if (datesAdjusted) {
      ...
      console.log(
        `      追加: ${datesAdded}日, 削除: ${datesRemoved}日 (day ${removedDays.join(", ")})`,
      );
    ...
    console.log(
      `   Match Status: ${matchStatus.matchCount} total, ${matchStatus.completedCount} completed, ${matchStatus.confirmedCount} confirmed`,
    );
    ...
    const changeTemplatePhaseMapping = buildTemplatePhaseMapping(
      changeTemplatePhases,
      newFormatPhases as string | null,
    );
    ...
    for (const template of templates.rows) {
      const actualPhase =
        changeTemplatePhaseMapping.get(template.phase as string) || (template.phase as string);
    ...
    for (const blockKey of allBlockKeys) {
      ...
      if (formatType === "tournament") {
        ...
        if (!blockMap.has(unifiedBlockKey)) {
          ...
          createdBlocks++;
          console.log(
            `   Created unified block: ${unifiedBlockName} (Phase: ${phase}, ID: ${unifiedBlockId})`,
          );
        }
      } else {
        // リーグ形式：個別ブロックを作成
    ...
    const fallbackDate =
      dayNumbers.length > 0
        ? tournamentDates[dayNumbers[0]]
        : String(new Date().toISOString().split("T")[0]);

    // 全試合の作成（予選・決勝共通処理）
    for (const template of templates.rows) {
      const actualTemplatePhase =
        changeTemplatePhaseMapping.get(template.phase as string) || (template.phase as string);
    ...
    try {
      ...
      if (missingPhases.length > 0) {
        ...
        console.log(
          `   ✅ ${missingPhases.length}件の不足ルール設定を追加: ${missingPhases.join(", ")}`,
        );
      }
      ...
      if (obsoletePhases.length > 0) {
        ...
        console.log(
          `   ✅ ${obsoletePhases.length}件の不要ルール設定を削除: ${obsoletePhases.join(", ")}`,
        );
      }
    } catch (ruleError) {
...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (matchStatus.hasResults) {
      reasons.push(
        `試合結果が既に入力されています（完了: ${matchStatus.completedCount}試合, 確定: ${matchStatus.confirmedCount}試合）`,
      );
    }

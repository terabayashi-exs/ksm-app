...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    const membersByTeam = new Map<
      string,
      Array<{ email: string; display_name: string; member_role: string }>
    >();
    ...
    // チームIDごとに送信履歴をグループ化
    const emailHistoryByTeam = new Map<
      number,
      Array<{ template_id: string; subject: string; sent_at: string }>
    >();
    ...
    const statistics = {
      confirmed: participants.filter(
        (p) =>
          p.participation_status === "confirmed" && p.withdrawal_status !== "withdrawal_requested",
      ).length,
      waitlisted: participants.filter((p) => p.participation_status === "waitlisted").length,
      withdrawal_requested: participants.filter(
        (p) => p.withdrawal_status === "withdrawal_requested",
      ).length,
    ...
    const participantsWithPosition = participants.map((p) => {
      if (p.participation_status === "waitlisted") {
        const position =
          waitlistedTeams.findIndex((t) => t.tournament_team_id === p.tournament_team_id) + 1;

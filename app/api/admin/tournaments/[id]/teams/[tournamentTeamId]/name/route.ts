...
export async function PUT(request: NextRequest, context: RouteContext) {
  try {
    ...
    const newOmission =
      team_omission && typeof team_omission === "string" && team_omission.trim() !== ""
        ? team_omission.trim()
        : newName;
    ...
    console.log(
      `[TEAM_NAME] Tournament ${tournamentId}, Team ${tournamentTeamId}: "${oldName}"(${oldOmission}) → "${newName}"(${newOmission}) by ${session.user.email}`,
    );
